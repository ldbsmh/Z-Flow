...
class FreeformView(
    ...
    //是否处于隐藏状态，当打开Z-Flow的正在运行小窗界面时，应当隐藏所有小窗
    var isHidden = false
    ...
    private val realScreenWidth: Int
        get() {
            ...
            val tmpWidth = rect.width()
            val tmpHeight = rect.height()
    ...
    private fun performBackKey() {
        runCatching {
            ...
            ZFlow.me.freeformManagerProxy.injectKeyEvent(KeyEvent.KEYCODE_BACK, virtualDisplay.display.displayId)
        }
    }
    ...
    override fun onScreenOff() {
        //挂起状态无需更新
        //修复 在有正在运行程序的情况下锁屏，Z-Flow崩溃的问题 q220902.1
                            ...
                            addListener (
                                onStart = {
                                    if (position != 0) {
                                        ...
                                        if (position == 1)
                                            hiddenView.findViewById<View>(R.id.backgroundView).background = ContextCompat.getDrawable(context, R.drawable.floating_button_bg_right)
    ...
    private inner class TouchListener : View.OnTouchListener{
        ...
        private fun handleTouch(event: MotionEvent) {
            ...
            ZFlow.me.freeformManagerProxy.injectMotionEvent(
        ...
        override fun onTaskDisplayChanged(tId: Int, newDisplayId: Int) {
            ...
            if (!isDestroy && taskList.contains(tId) && newDisplayId == Display.DEFAULT_DISPLAY) {
                if (config.useSuiRefuseToFullScreen)
                    ZFlow.me.freeformManagerProxy.moveTaskToDisplay(tId, virtualDisplay.display.displayId)
                else
                    context.startService(
                        Intent(context, FreeformService::class.java)
                            .setAction(FreeformService.ACTION_CALL_INTENT)
